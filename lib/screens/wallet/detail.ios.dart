...
class WalletDetailIOSScreen extends BaseStateFulWidget {
  final Map<String, dynamic>? arguments;

  const WalletDetailIOSScreen({Key? key, this.arguments}) : super(key: key);

  @override
  _WalletDetailIOSScreenState createState() => _WalletDetailIOSScreenState();
}

class _WalletDetailIOSScreenState extends BaseStateFulWidgetState<WalletDetailIOSScreen> {
  ...
  void onRefreshArguments() {
    this._wallet = widget.arguments?['wallet'];
  }
  ...
  void initState() {
    ...
    if (this._wallet?.type == WalletType.eth) {
      walletCommon.queryETHBalance(this._wallet!, notifyIfNeed: true, delayMs: 500);
    } else if (this._wallet?.type == WalletType.nkn) {
      walletCommon.queryNKNBalance(this._wallet!, notifyIfNeed: true, delayMs: 500);
    }
  }
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          Label('NKN', type: LabelType.bodySmall, color: application.theme.fontColor1),
                        ],
                      ),
