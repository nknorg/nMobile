...
class Signature {
  ...
  Uint8List open_off(Uint8List signedMessage, final int smoff) {
    if (!(signedMessage.length > smoff)) return throw ArgumentError();
  ...
  Uint8List open_len(Uint8List signedMessage, final int smoff, final int smlen) {
    ...
    if (0 != TweetNaclFast.crypto_sign_open(tmp, -1, signedMessage, smoff, smlen, _theirPublicKey)) return throw ArgumentError();
