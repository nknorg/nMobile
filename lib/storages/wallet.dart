...
class WalletStorage {
  static const String KEY_WALLETS = 'WALLETS';
  ...
  Future<List> getWallets() async {
    var wallets = await _localStorage.getArray(KEY_WALLETS);
  ...
  Future addWallet(WalletSchema walletSchema, String keystore, {String password, String seed}) async {
    ...
    if (index < 0) {
      futures.add(_localStorage.addItem(KEY_WALLETS, walletSchema?.toCacheMap()));
    } else {
      futures.add(_localStorage.setItem(KEY_WALLETS, index, walletSchema?.toCacheMap()));
    }
  ...
  Future deleteWallet(int n, WalletSchema walletSchema) async {
    List<Future> futures = <Future>[];
    if (n >= 0) {
      futures.add(_localStorage.removeItem(KEY_WALLETS, n));
      if (Platform.isAndroid) {
  ...
  Future updateWallet(int n, WalletSchema walletSchema, {String keystore, String password, String seed}) {
    List<Future> futures = <Future>[];
    if (n >= 0) {
      futures.add(_localStorage.setItem(KEY_WALLETS, n, walletSchema?.toCacheMap()));
      if (keystore != null && keystore.isNotEmpty) {
        if (Platform.isAndroid) {
          futures.add(_localStorage.set('$KEY_KEYSTORE:${walletSchema?.address}', keystore));
        } else {
