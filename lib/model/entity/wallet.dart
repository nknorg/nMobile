...
class WalletSchema extends Equatable with Tag {
  ...
  Future<String> _showDialog(String reason) {
    return BottomDialog.of(Global.appContext).showInputPasswordDialog(
        title: NL10ns.of(Global.appContext).verify_wallet_password);
  }

  Future<String> getPassword(
      {bool showDialogIfCanceledBiometrics = true,
      bool forceShowInputDialog = false}) async {
    if (forceShowInputDialog) {
      return _showDialog('force');
    }

    bool underProtection =
        await LocalAuthenticationService.instance.protectionStatus();
    if (underProtection) {
      String password =
          await _secureStorage.get('${SecureStorage.PASSWORDS_KEY}:$address');
  ...
  Future<String> getKeystoreByPassword(String password) async {
    String keystore =
        await LocalStorage().getValueDecryptByKey(password, address);
  ...
  Future<String> getKeystore() async {
    if (Platform.isAndroid) {
      ...
      if (keyStore == null || keyStore.length == 0) {
        /// if not new Comer
        keyStore = await _secureStorage
            .get('${SecureStorage.NKN_KEYSTORES_KEY}:$address');
        if (keyStore == null) {
          /// keystore is broken
          keyStore = '';
        } else {
  ...
  Future<Map> exportWallet(password) async {
    ...
    await _secureStorage.set(
        '${SecureStorage.PASSWORDS_KEY}:$address', password);
  ...
  static getWallet({int index = 0}) async {
    try {
      ...
      var wallet = WalletSchema(
          name: walletData['name'], address: walletData['address']);
      return wallet;
    } catch (e) {
      return null;
    }
