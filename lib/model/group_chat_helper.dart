...
class GroupChatPublicChannel {
  ...
  //   try {
  //     NLog.w('pullSubscribersPublicChannel called');
  //     final topicHashed = genTopicHash(topicName);
  //
  ...
  //             txPool: true)
  //         .then((subscribersMap) async {
  //           NLog.w('subscribersMap is_____'+subscribersMap.toString());
  ...
  //         }
  //         NLog.w('Insert Members___'+dataList.length.toString()+'__'+topicName);
...
class GroupChatPrivateChannel {
  ...
  static Future<void> uploadPermissionMeta(
      ...
      BlackListRepo repoBlackL}) async {
    return;
    // if (_topicIsUploading.containsKey(topicName) &&
    //     _topicIsUploading[topicName]) return;
    ...
    //   NLog.w('___________'+accountPubkey.toString());
    //   Map<String, dynamic> makePrmJson(
    //       List<Subscriber> paged, List<BlackList> pagedBlack) {
    //     try {
    ...
    //           final arr = [];
    //           arr.add({
    //             (/*isPubkey(subs.chatIdOrPubkey) ? pubkey :*/ addr):
    //                 subs.chatIdOrPubkey
    //           });
    //           json[reject] = arr;
    //           first = false;
    //         } else {
    //           (json[reject] as List).add({
    //             (/*isPubkey(subs.chatIdOrPubkey) ? pubkey :*/ addr):
    //                 subs.chatIdOrPubkey
    //           });
    //         }
    //       });
    //       NLog.w('UpdatePer____'+json.toString());
    //       return json;
    //     } catch (e) {
    ...
    //   if (maxPageIndex < 0) {
    //     NLog.w(
    //         'uploadPermissionMeta | [--, $maxPageIndex] whiteList:\n${whiteList.length}');
    //   }
    //   var pageIndex = 0;
    ...
    //     if (paged.any((e) => !e.uploaded) ||
    //         pagedBlack.any((e) => !e.uploaded)) {
    ...
    //   temp1.removeWhere((s) =>
    //       newAddedBlack.any((it) => s.chatIdOrPubkey == it.chatIdOrPubkey));
    //   newAddedBlack.addAll(temp1);
    //
    //   List<Subscriber> left = [];
    //   List<BlackList> leftBlack = [];
    //   while (newAdded.any((e) => !e.uploaded) ||
    //       newAddedBlack.any((e) => !e.uploaded)) {
    ...
    //           newAddedBlack =
    //               newAddedBlack.sublist(0, newAddedBlack.length - 1);
    //         }
    //       } else {
    //         for (var it in newAdded) {
    //           await repoSub.updatePermiPageIndex(
    //               topicName, it.chatId, pageIndex);
    //         }
    //         for (var it in newAddedBlack) {
    //           await repoBlackL.updatePermiPageIndex(
    //               topicName, it.chatIdOrPubkey, pageIndex);
    //         }
  ...
  static Future<int> pullSubscribersPrivateChannel(
    ...
    //     return blackListChatId.containsKey(chatId) ||
    //         blackListPubkey.keys.any((k) => chatId.endsWith(k));
    //   }
    //
    //   bool notInBlackList(String chatId) => !inBlackListFunc(chatId);
    //
    //   bool inWhiteListFunc(String chatId) {
    //     return whiteListChatId.containsKey(chatId) ||
    //         whiteListPubkey.keys.any((k) => chatId.endsWith(k));
    //   }
    //
    ...
    //               topicHash: topicHashed,
    //               subscriber: NKNClientCaller.currentChatId);
    //       await repoTopic.updateOwnerExpireBlockHeight(
    //           topicName, subscription['expiresAt']);
    //     } catch (e) {
    //       NLog.e(
    //           "pullSubscribersPrivateChannel | updateOwnerExpireBlockHeight. e:" +
    //               e.toString());
    //     }
    //   }
    ...
    //     if (!isPrmCtl &&
    //         (acceptAll ||
    //             (notInBlackList(chatId) && (inWhiteList || ownerIsMe)))) {
    //       if (!acceptAll && !inWhiteList) {
    //         // this case indicates `ownerIsMe`
    //         final subscriber =
    //             await repoSub.getByTopicAndChatId(topicName, chatId);
    //         if (subscriber == null) {
    //           needUploadMeta = true;
    ...
    //         uploaded: true,
    //         subscribed:
    //             chatIdsCurrentlySubscribed.any((e) => e.endsWith(pubkey))));
    //   }
    //
    //   /// 3.2. Save the chatId (not included in the pubkey) in the permission-controlled
    //   /// blacklist into the blacklist data table.
    //   Map<String, int> temp = Map.of(blackListChatId);
    //   temp.removeWhere((chatId, _) =>
    //       blackListPubkey.keys.any((pubkey) => chatId.endsWith(pubkey)));
    ...
    //   /// And end to return.
    //   if (!ownerIsMe &&
    //       (!chatIdsCurrentlySubscribed
    //               .contains(NKNClientCaller.currentChatId) ||
    //           inBlackListFunc(NKNClientCaller.currentChatId))) {
    ...
    //     whiteList.forEach((w) {
    //       if (blackList.any((b) =>
    //           w.chatId == b.chatIdOrPubkey ||
    //           w.chatId.endsWith(b.chatIdOrPubkey))) {
    //         beMixed.add(w);
    //       }
    //     });
    //
    ...
    //     temp.removeWhere(
    //         (it) => chatIdsCurrentlySubscribed.contains(it.chatId));
    //     for (var it in temp) {
    //       await repoSub.delete(topicName, it.chatId);
    //     }
    //
