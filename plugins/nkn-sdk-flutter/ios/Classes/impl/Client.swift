...
class Client : ChannelBase, IChannelHandler, FlutterStreamHandler {
    ...
    let clientMessageEventQueue = DispatchQueue(label: "org.nkn.sdk/client/messageEvent/queue", qos: .default, attributes: .concurrent)
    private var clientMessageEventWorkItem: DispatchWorkItem?
    ...
    private func addMessageEventQueue(client: NknMultiClient) {
        clientMessageEventWorkItem = DispatchWorkItem {
            self.onMessageEvent(client: client)
        }
        clientMessageEventQueue.async(execute: clientMessageEventWorkItem!)
    }
    
    private func onMessageEvent(client: NknMultiClient) {
        guard let event = client.onMessageEvent?.next() else {
            return
        }
        
        var resp: [String: Any] = [String: Any]()
        resp["_id"] = client.address()
        resp["event"] = "onMessageEvent"
        
        var data: [String: Any] = [String: Any]()
        data["type"] = event.type
        data["clientAddr"] = event.clientAddr
        data["subClientID"] = event.subClientID
        if let destinations = event.destinations {
            var destArray: [String] = []
            for i in 0..<destinations.len() {
                destArray.append(destinations.get(i))
            }
            data["destinations"] = destArray
        } else {
            data["destinations"] = nil
        }
        data["src"] = event.src
        data["messageId"] = event.messageID != nil ? FlutterStandardTypedData(bytes: event.messageID!) : nil
        data["messageType"] = event.messageType
        data["encrypted"] = event.encrypted
        data["dataSize"] = event.dataSize
        data["noReply"] = event.noReply
        data["timestamp"] = event.timestamp
        if event.error != nil {
            data["error"] = event.error!.localizedDescription
        }
        
        resp["data"] = data
        NSLog("%@", resp)
        self.eventSinkSuccess(eventSink: eventSink, resp: resp)
        self.addMessageEventQueue(client: client)
    }
    ...
    private func getSubClientConnectionStates(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        guard let multiClient = getClient(id: _id) else {
            result([])
            return
        }
        var list: [[String: Any]] = []
    ...
    private func create(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        let crossSendPolicy = args["crossSendPolicy"] as? Int
        
        let config: NknClientConfig = NknClientConfig()
        if (crossSendPolicy != nil) {
            config.crossSendPolicy = Int32(crossSendPolicy!)
        }
        if(seedRpc != nil){
